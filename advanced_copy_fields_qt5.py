...
class UAdvancedCopy(QDialog):
    ...
    def _setupUi(self):
        ...
        spacerItem_cmbSource = QtWidgets.QSpacerItem(50, 20, QtWidgets.QSizePolicy.Expanding,
        ...
        spacerItem1 = QtWidgets.QSpacerItem(50, 20, QtWidgets.QSizePolicy.Expanding,
                                            QtWidgets.QSizePolicy.Minimum)  # HELLO HERE
    ...
    def onConfirm(self):
        ...
        if idx == 0:  # If Replace
            q = (
                "The contents of the field '{1}' will be replaced. The field '{0}' will replace it in {2} selected note(s).<br><br>Is this okay?").format(
                fld1, fld2, len(nids))
        elif idx == 1:  # Copy After
            q = (
                "The contents of the field '{1}' will change. The field '{0}' will be appended to it in {2} selected note(s).<br><br>Is this okay?").format(
                fld1, fld2, len(nids))
        elif idx == 2:  # Copy Before
            q = (
                "The contents of the field '{1}' will change. The field '{0}' will be prepended to it in {2} selected note(s).<br><br>Is this okay?").format(
                fld1, fld2, len(nids))
        elif idx == 3:  # Move
            q = (
                "The contents of the field '{0}' will move to field '{1}'. This will replace out field '{1}' but also empty out field '{0}' in {2} selected note(s).<br><br>Is this okay?").format(
                fld1, fld2, len(nids))
        elif idx == 4:  # Swap
            if fld1 == fld2:
                QMessageBox.warning(self, "Swap Error", (
                    "You must select two different fields in order to swap them. You selected '{0}' in both boxes.").format(
                    fld1))
                return
            q = (
                "The contents of the fields '{0}' and '{1}' will be swapped. The contents of '{0}' will become '{1}' and '{1}' will become '{0}', in {2} selected note(s).<br><br>Is this okay?").format(
                fld1, fld2, len(nids))
        elif idx == 5:  # Custom
            q = (
                "The contents of the field '{1}' will be replaced with the processed contents of the custom template in {2} selected note(s).<br><br>Is this okay?").format(
                fld1, fld2, len(nids))
...
def setup_menu(browser):
    ...
    a.setShortcut(QKeySequence(get_shortcut()))
    a.setObjectName("actionAdvancedCopy")
